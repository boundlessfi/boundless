...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const blogPosts = fetchBlogPostsSitemap();
  const [hackathons, crowdfundingProjects] = await Promise.all([
...
/**
 * Reads blog posts from MDX files and formats them for sitemap
 */
function fetchBlogPostsSitemap(): MetadataRoute.Sitemap {
  try {
