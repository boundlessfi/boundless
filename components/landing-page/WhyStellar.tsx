...
const Reason = ({ icon, title, body }: ReasonProps) => (
  <div className='border-border-subtle bg-stage flex flex-col gap-3 rounded-xl border p-5'>
    <div className='border-border-subtle bg-surface flex h-9 w-9 items-center justify-center rounded-lg border'>
      <div className='text-foreground/75'>{icon}</div>
    </div>
    <h3 className='text-foreground text-base font-semibold md:text-lg'>
      {title}
    </h3>
    <p className='text-secondary-text text-sm leading-[160%]'>{body}</p>
  </div>
);
      ...
      <div className='border-border-subtle bg-surface relative overflow-hidden rounded-[24px] border p-8 md:p-12 lg:p-16'>
        <div className='relative grid gap-12 lg:grid-cols-[5fr_7fr] lg:gap-16'>
          <header className='flex flex-col justify-center'>
            <p className='text-mint text-sm font-medium tracking-wide uppercase'>
              Built on Stellar
            </p>
            <h2
              id='why-stellar-heading'
              className='text-foreground mt-3 text-3xl leading-[120%] tracking-tight md:text-4xl xl:text-[44px]'
            >
              Why we built this on Stellar
            </h2>
            <p className='text-secondary-text mt-5 text-base leading-[170%] md:text-lg'>
              Boundless runs on Stellar for four specific reasons. Funds release
              at human speed, fees stay out of the way, USDC works without
              bridges, and every program has a working dispute path.
            </p>
            <Link
              href='/why-stellar'
              className='text-foreground hover:text-mint group mt-6 inline-flex items-center gap-2 self-start text-sm font-medium underline-offset-4 hover:underline'
            >
              Read more on Stellar and Soroban
